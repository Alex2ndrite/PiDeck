...
- [ ] 2.1 版本号对齐：分支改为 `0.7.0-beta.0`（明确这是架构切换版本，区别于 main 0.6.7）
- [ ] 2.2 把本文档的 parity 表复制到 issue #113 评论，逐项勾选
- [ ] 2.3 跑全量现有单测：`npm test`，记录当前通过率作为基线
- [ ] 2.4 跑 `npm run typecheck`，必须 0 error
...
1. 冷启动 → 窗口按设置的启动大小打开
2. 单实例开启时二次启动 → 聚焦已有窗口（含托盘唤起）
3. 修改沙箱开关 → 重启后应用启动正常
4. 修改 pi RPC 启动 flags → 重启 Agent 后生效
