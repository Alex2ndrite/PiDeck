...
- **系统通知点击直达对应会话** — 通知点击优先按会话 `record.id` 定位并切换到对应
  会话，不再只落在当前会话。
- **启动引导自动选中首个项目** — 引导流程延迟创建草稿会话，首项目自动选中。
- **起始页统一居中** — 无会话起始页的 ComposerArea 统一居中，不再受残留布局偏移。
...
- **流式回复滚动跟随不再中途失灵** — 流式渲染追底存在弹簧滞后时，轻微上滚（含触控板
  惯性/误触）不再被误判为「用户离开底部」，回复持续贴底滚动；即使已误判，用户仍停在
  近底部时内容增长会自动恢复跟随，不再需要手动点「回到底部」按钮。
