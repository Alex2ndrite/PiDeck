...
const lxgwStyles = readFileSync("src/renderer/src/styles/lxgw-wenkai.css", "utf8");
...
test("config shell defines compact density and crisp system typography", () => {
  ...
  assert.match(foundation, /LXGW WenKai/);
  assert.match(foundation, /Microsoft YaHei UI/);
  assert.doesNotMatch(foundation, /MiSans/);
  assert.match(settingsModal, /value: "lxgw-wenkai"/);
  assert.match(settingsModal, /value: "system"/);
  assert.match(rendererStyles, /styles\/lxgw-wenkai\.css/);
  assert.match(lxgwStyles, /LXGWWenKai-Medium\.ttf/);
  assert.doesNotMatch(lxgwStyles, /LXGWWenKai-(Light|Regular)\.ttf/);
