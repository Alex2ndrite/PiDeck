...
test("SessionRuntimeDock retains the mounted terminal through the closing animation", () => {
  const source = readFileSync(
    "src/renderer/src/components/session/SessionRuntimeDock.tsx",
    "utf8",
  );
  assert.match(source, /const \[mounted, setMounted\] = useState\(false\)/);
  assert.match(source, /const \[closing, setClosing\] = useState\(false\)/);
  assert.match(source, /SESSION_RUNTIME_DOCK_MOTION_MS = 180/);
  assert.match(source, /window\.setTimeout\([\s\S]*setMounted\(false\)[\s\S]*SESSION_RUNTIME_DOCK_MOTION_MS/);
